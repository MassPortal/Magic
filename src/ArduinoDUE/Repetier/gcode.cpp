...
void GCode::readFromSerial()
{
    ...
    if(!Serial.available())
    {
        ...
        else if(time - timeOfLastDataPacket > 1000)   // Don't do it if buffer is not empty. It may be a slow executing command.
        {
...
bool GCode::parseAscii(char *line,bool fromSerial)
{
    ...
    while ( (c = *(pos++)) )
    {
        ...
        case 'm':
        {
            ...
            if (M == 20 || M == 23 || M == 28 || M == 29 || M == 30 || M == 32 || M == 36 || M == 117 || M == 887 )
            {
                ...
                while (*pos)
                {
                    if((M != 887 && M != 117 && M != 20 && *pos==' ') || *pos=='*') break;
