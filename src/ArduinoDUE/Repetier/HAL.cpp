...
void beginAsync(uint8_t id, int32_t steps, bool (*fun)(void))
{
    ...
    if (first) {
        ...
        /* Set frequency etc */
        TC_Configure(EXTRUDER_TIMER, EXTRUDER_TIMER_CHANNEL, TC_CMR_WAVSEL_UP_RC|TC_CMR_WAVE|TC_CMR_TCCLKS_TIMER_CLOCK3);
        TC_SetRC(EXTRUDER_TIMER, EXTRUDER_TIMER_CHANNEL, (F_CPU_TRUE / 32) / EXTRUDER_CLOCK_FREQ);
        /* Enable interrupt */
        EXTRUDER_TIMER->TC_CHANNEL[EXTRUDER_TIMER_CHANNEL].TC_IER = TC_IER_CPCS;
...
void EXTRUDER_TIMER_VECTOR(void)
{
    ...
    if (!moveing) TC_Stop(EXTRUDER_TIMER, EXTRUDER_TIMER_CHANNEL);
}
